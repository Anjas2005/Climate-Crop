...
def analyze_crop_health(temperature, rainfall):
    """Generate crop health score based on temperature and rainfall."""
    # Scale: 0 (poor health) to 100 (optimal health)
    temp_score = max(0, 100 - abs(temperature - 25) * 5)
    rain_score = max(0, 100 - abs(rainfall - 75) * 1.5)
...
def main():
    st.title("Weather and Crop Health Prediction in Bengaluru")
    ...
    if weather_data:
        ...
        if not filtered_df.empty:
            ...
            crop_health = analyze_crop_health(avg_temp, total_rainfall)
            st.write(f"**Crop Health Score:** {crop_health:.2f} (Scale: 0-100)")
            ...
            # Crop Health Graph
            st.subheader("Crop Health Prediction Graph")
            health_fig = px.line(filtered_df, x="date", y=filtered_df["avg_temperature"].apply(lambda t: analyze_crop_health(t, total_rainfall)),
